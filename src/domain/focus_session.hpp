...
struct FocusSession {
    ...
    bool IsActive()   const { return status == SessionStatus::kActive; }
    bool IsPaused()   const { return status == SessionStatus::kPaused; }
    ...
    int NetFocusMinutes() const {
        ...
        for (const auto& br : breaks)
            if (br.duration_minutes) break_min += *br.duration_minutes;
        return std::max(0, actual_duration_minutes - break_min);
    }

    /// Процент выполнения плана (0–100)
    double CompletionPercent() const {
        if (planned_duration_minutes == 0) return 0.0;
        return std::min(100.0,
            100.0 * actual_duration_minutes / planned_duration_minutes);
    }
};
