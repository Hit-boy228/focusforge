...
inline TaskStatus TaskStatusFromString(const std::string& s) {
    if (s == "new")
        return TaskStatus::kNew;
    if (s == "in_progress")
        return TaskStatus::kInProgress;
    if (s == "paused")
        return TaskStatus::kPaused;
    if (s == "done")
...
inline std::string ToString(SessionMode m) {
    switch (m) {
        case SessionMode::kPomodoro:
            return "pomodoro";
        case SessionMode::kDeepWork:
            return "deep_work";
        case SessionMode::kCustom:
            return "custom";
    }
    return "custom";
}
...
inline SessionMode SessionModeFromString(const std::string& s) {
    if (s == "pomodoro")
        return SessionMode::kPomodoro;
    if (s == "deep_work")
        return SessionMode::kDeepWork;
    if (s == "custom")
        return SessionMode::kCustom;
...
inline std::string ToString(SnoozeReason r) {
    switch (r) {
        case SnoozeReason::kBusy:
            return "busy";
        case SnoozeReason::kWaitingReply:
            return "waiting_reply";
        case SnoozeReason::kLowEnergy:
            return "low_energy";
        case SnoozeReason::kNotReady:
            return "not_ready";
        case SnoozeReason::kOther:
            return "other";
    }
    return "other";
}
...
inline SnoozeReason SnoozeReasonFromString(const std::string& s) {
    if (s == "busy")
        return SnoozeReason::kBusy;
    if (s == "waiting_reply")
        return SnoozeReason::kWaitingReply;
    if (s == "low_energy")
        return SnoozeReason::kLowEnergy;
    if (s == "not_ready")
        return SnoozeReason::kNotReady;
