...
struct Task {
    ...
    bool IsOverdue(Timestamp now = Now()) const {
        return deadline.has_value()
            && *deadline < now
            && status != TaskStatus::kDone
            && status != TaskStatus::kArchived;
    }

    bool IsCompleted() const { return status == TaskStatus::kDone; }
    bool IsActive() const {
        return status == TaskStatus::kNew
            || status == TaskStatus::kInProgress;
    }
    ...
    double AgingRiskScore(Timestamp now = Now()) const {
        ...
        if (deadline.has_value()) {
            auto hours_to_dl = std::chrono::duration_cast<std::chrono::hours>(
                *deadline - now).count();
            if (hours_to_dl < 0)        base *= 3.0;   // просрочена
            else if (hours_to_dl < 24)  base *= 2.0;   // сегодня
        }
    ...
    int SubtasksCompleted() const {
        int c = 0;
        for (const auto& s : subtasks) if (s.is_done) ++c;
        return c;
    }

    double SubtaskCompletionRate() const {
        if (subtasks.empty()) return 1.0;
