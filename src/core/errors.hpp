...
public:
    ValidationError(std::string_view field, std::string_view msg)
        : DomainError("Validation failed for '" + std::string(field)
                      + "': " + std::string(msg)) {}
...
public:
    RateLimitError(int limit, int window_sec)
        : DomainError("Rate limit exceeded: " + std::to_string(limit)
                      + " req/" + std::to_string(window_sec) + "s") {}
};

class LimitExceededError : public DomainError {
public:
    LimitExceededError(std::string_view resource, int current, int max)
        : DomainError(std::string(resource) + " limit exceeded: "
