...
template <typename T>
std::optional<T> GetOpt(const userver::formats::json::Value& j, const std::string& key) {
    try {
        if (!j.HasMember(key))
            return std::nullopt;
        return j[key].As<T>();
...
inline std::optional<userver::formats::json::Value> ParseSafe(const std::string& s) {
    try {
        return userver::formats::json::FromString(s);
    } catch (...) {
        return std::nullopt;
    }
}
...
inline userver::formats::json::Value EmptyObject() {
    return userver::formats::json::ValueBuilder{userver::formats::json::Type::kObject}
        .ExtractValue();
}
