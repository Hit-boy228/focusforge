...
domain::Reminder ReminderRepository::Insert(const dto::CreateReminderRequest& req) {
    ...
    auto res = pg_->Execute(pg::ClusterHostType::kMaster, kQ,
        core::GenerateUuid(), req.user_id, req.task_id,
        req.message, req.remind_at_iso,
        req.is_recurring, req.recurrence_rule);
...
std::vector<domain::Reminder> ReminderRepository::FindDue(int limit) {
    ...
    for (const auto& r : res) reminders.push_back(MapRow(r));
    return reminders;
}

std::vector<domain::Reminder> ReminderRepository::FindByUser(
...
void ReminderRepository::MarkSent(const std::string& id) {
    pg_->Execute(pg::ClusterHostType::kMaster,
        "UPDATE reminders SET is_sent=TRUE, sent_at=NOW(), send_attempts=send_attempts+1 WHERE id=$1::uuid",
        id);
}

void ReminderRepository::Snooze(const std::string& id,
                                  const std::string& until_iso,
                                  const std::string& user_id,
                                  int snooze_minutes,
                                  std::optional<std::string> reason) {
    pg_->Execute(pg::ClusterHostType::kMaster,
        "UPDATE reminders SET remind_at=$2::timestamptz WHERE id=$1::uuid",
        id, until_iso);
    pg_->Execute(pg::ClusterHostType::kMaster,
...
domain::Reminder ReminderRepository::MapRow(const pg::Row& r) {
    ...
    if (!r["task_id"].IsNull()) rem.task_id = r["task_id"].As<std::string>();
