...
std::optional<domain::Goal> GoalRepository::FindCurrentGoal(
...
domain::Goal GoalRepository::Upsert(const domain::Goal& goal) {
    ...
    auto res = pg_->Execute(pg::ClusterHostType::kMaster, kQ,
        goal.id.empty() ? core::GenerateUuid() : goal.id,
        goal.user_id, goal.type,
        goal.target_focus_minutes, goal.target_tasks_count,
        goal.period_start, goal.period_end,
        goal.achieved_focus_minutes, goal.achieved_tasks_count);
    const auto& r = res.Front();
    domain::Goal g = goal;
    g.id = r["id"].As<std::string>();
    return g;
...
void GoalRepository::IncrementAchievedFocus(
    const std::string& user_id, const std::string& type, int minutes) {
...
void GoalRepository::IncrementAchievedTasks(
    const std::string& user_id, const std::string& type, int count) {
    pg_->Execute(pg::ClusterHostType::kMaster, R"~(
