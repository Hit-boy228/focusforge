...
std::optional<domain::Task> TaskRepository::FindById(
...
domain::Task TaskRepository::Insert(
    pg::Transaction& trx, const domain::Task& t) {
    static constexpr auto kQ = R"~(
    ...
    auto res = trx.Execute(kQ,
        core::GenerateUuid(), t.user_id, t.parent_task_id,
        t.title, t.description,
        domain::ToString(t.status), domain::ToString(t.priority),
        t.deadline, t.estimated_minutes,
        t.is_recurring, t.recurrence_rule);
    return MapRow(res.Front());
}

std::optional<domain::Task> TaskRepository::UpdateWithVersion(
    pg::Transaction& trx, const domain::Task& t, int expected_version) {
    static constexpr auto kQ = R"~(
        UPDATE tasks SET
            title             = $3,
            description       = $4,
    ...
    auto res = trx.Execute(kQ,
        ...
        domain::ToString(t.status), domain::ToString(t.priority),
        t.deadline, t.estimated_minutes, expected_version);
    if (res.IsEmpty()) return std::nullopt;  // version mismatch
    return MapRow(res.Front());
}

bool TaskRepository::SoftDelete(
    pg::Transaction& trx, const std::string& id, const std::string& user_id) {
...
std::tuple<std::vector<domain::Task>, int> TaskRepository::FindWithFilter(
    ...
    if (f.status) status_param = domain::ToString(*f.status);

    std::optional<std::string> priority_param;
    if (f.priority) priority_param = domain::ToString(*f.priority);
    ...
    auto res = pg_->Execute(pg::ClusterHostType::kSlave, kQuery,
        f.user_id, status_param, priority_param, search_param,
        overdue_only, f.Limit(), f.Offset());

    std::vector<domain::Task> tasks;
    tasks.reserve(res.Size());
    for (const auto& row : res) tasks.push_back(MapRow(row));
    ...
    auto cnt_res = pg_->Execute(pg::ClusterHostType::kSlave, kCountQuery,
        f.user_id, status_param, priority_param, search_param, overdue_only);
...
int TaskRepository::CountByUser(const std::string& user_id) {
    auto res = pg_->Execute(pg::ClusterHostType::kSlave,
        "SELECT COUNT(*) FROM tasks WHERE user_id=$1::uuid AND NOT is_deleted", user_id);
    return res.Front()[0].As<int>();
}

int TaskRepository::CountCompletedInPeriod(const std::string& user_id,
...
domain::Task TaskRepository::ChangeStatus(const std::string& id,
                                            const std::string& user_id,
                                            domain::TaskStatus new_status,
                                            int expected_version) {
    auto& pg = *pg_;
    auto trx = pg_->Begin("change_status",
    ...
    trx.Commit();
    if (!updated) throw core::ConflictError("Version mismatch or task not found: " + id);
    return *updated;
}
