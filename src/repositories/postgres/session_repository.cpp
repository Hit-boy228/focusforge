...
domain::FocusSession SessionRepository::Insert(
    pg::Transaction& trx, const domain::FocusSession& s) {
    auto res = trx.Execute(sql::kInsertSession,
        core::GenerateUuid(), s.user_id, s.task_id,
...
std::optional<domain::FocusSession> SessionRepository::FindById(
...
domain::FocusSession SessionRepository::Update(
    pg::Transaction& trx, const domain::FocusSession& s) {
    std::optional<domain::Timestamp> paused_at = s.paused_at;
    std::optional<domain::Timestamp> ended_at  = s.ended_at;
    auto res = trx.Execute(sql::kUpdateSession,
        s.id, domain::ToString(s.status),
        s.actual_duration_minutes, s.completed_pomodoros,
        s.interruption_count, s.focus_debt_minutes,
        s.notes, paused_at, ended_at);
    return MapRow(res.Front());
}

int SessionRepository::SumFocusMinutes(const std::string& user_id,
                                        const std::string& date_from,
                                        const std::string& date_to) {
    auto res = pg_->Execute(pg::ClusterHostType::kSlave, sql::kSumFocusMinutes,
        user_id, date_from, date_to);
    return res.Front()[0].As<int>();
}

std::vector<domain::FocusSession> SessionRepository::FindCompleted(
    const std::string& user_id,
    const std::string& date_from,
    const std::string& date_to) {
    ...
    for (const auto& row : res) sessions.push_back(MapRow(row));
    return sessions;
}
...
domain::FocusSession SessionRepository::MapRow(const pg::Row& r) {
    ...
    if (!r["task_id"].IsNull()) s.task_id = r["task_id"].As<std::string>();
    ...
    if (!r["notes"].IsNull()) s.notes = r["notes"].As<std::string>();
    s.started_at  = r["started_at"].As<domain::Timestamp>();
    if (!r["paused_at"].IsNull()) s.paused_at = r["paused_at"].As<domain::Timestamp>();
    if (!r["ended_at"].IsNull())  s.ended_at  = r["ended_at"].As<domain::Timestamp>();
