...
class TaskRepository final : public userver::components::ComponentBase {
   public:
    ...
    std::optional<domain::Task> FindById(const std::string& id, const std::string& user_id);
    domain::Task Insert(userver::storages::postgres::Transaction& trx, const domain::Task& task);
    std::optional<domain::Task> UpdateWithVersion(userver::storages::postgres::Transaction& trx,
                                                  const domain::Task& task, int expected_version);
    bool SoftDelete(userver::storages::postgres::Transaction& trx, const std::string& id,
                    const std::string& user_id);

    /// Возвращает {tasks, total}
    std::tuple<std::vector<domain::Task>, int> FindWithFilter(const dto::TaskFilterRequest& filter);

    int CountByUser(const std::string& user_id);
    int CountCompletedInPeriod(const std::string& user_id, const std::string& date_from,
                               const std::string& date_to);

    domain::Task ChangeStatus(const std::string& id, const std::string& user_id,
