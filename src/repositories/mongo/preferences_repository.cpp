...
PreferencesRepository::PreferencesRepository(
    ...
    : ComponentBase(cfg, ctx),
      mongo_(
          ctx.FindComponent<userver::components::Mongo>(
                 "mongo-focusforge")
              .GetPool()) {}

std::optional<userver::formats::json::Value>
PreferencesRepository::Get(const std::string& user_id) {
    ...
    try {
        const auto json =
            userver::formats::bson::ToCanonicalJsonString(*doc).ToString();

        return userver::formats::json::FromString(json);
    } catch (const std::exception& e) {
        LOG_WARNING()
            << "Preferences deserialization failed: "
            << e.what();
...
void PreferencesRepository::Upsert(
    ...
    const auto prefs_json =
        userver::formats::json::ToString(prefs);

    bson::ValueBuilder prefs_bson;
    prefs_bson = bson::FromJsonString(prefs_json);

    set_doc["preferences"] =
        prefs_bson.ExtractValue();
...
void PreferencesRepository::PatchField(
    const std::string& user_id,
    ...
    const auto value_json =
        userver::formats::json::ToString(value);

    bson::ValueBuilder value_bson;
    value_bson = bson::FromJsonString(value_json);

    bson::ValueBuilder set_doc;
    set_doc["preferences." + field_path] =
        value_bson.ExtractValue();
