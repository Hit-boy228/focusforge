...
void ReportSnapshotRepository::Save(
    ...
    const auto data_json =
        userver::formats::json::ToString(data);
...
ReportSnapshotRepository::Get(
    ...
    try {
        const auto json =
            userver::formats::bson::ToCanonicalJsonString(
                *doc).ToString();

        return userver::formats::json::FromString(json);
    } catch (const std::exception& e) {
        LOG_WARNING()
            << "Report snapshot deserialization failed: "
            << e.what();
