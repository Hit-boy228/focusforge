...
std::vector<EventLogEntry> EventLogRepository::FindByUser(
    ...
    auto cursor = coll.Find(filter.ExtractValue(),
    ...
    for (auto& doc : cursor) {
        ...
        e.event_type = doc.HasMember("event_type") ? doc["event_type"].As<std::string>() : std::string{};
