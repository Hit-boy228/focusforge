...
RateLimitRepository::RateLimitRepository(const userver::components::ComponentConfig& cfg,
                                         const userver::components::ComponentContext& ctx)
    : ComponentBase(cfg, ctx)
    , redis_(ctx.FindComponent<userver::components::Redis>("redis-focusforge")
                 .GetClient("focusforge")) {}

int64_t RateLimitRepository::Increment(int64_t user_id, std::chrono::seconds window) {
