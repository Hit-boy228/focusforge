...
dto::InlineKeyboardMarkup KeyboardBuilder::MainMenu() {
    return {
        {Btn{"📋 Задачи",    "menu:tasks",   ""},
         Btn{"➕ Новая",     "menu:newtask", ""}},
        {Btn{"🎯 Фокус",     "menu:focus",   ""},
         Btn{"📊 Статистика","menu:stats",   ""}},
        {Btn{"📅 План дня",  "menu:plan",    ""},
         Btn{"⚙️ Настройки","menu:settings",""}},
    };
}

dto::InlineKeyboardMarkup KeyboardBuilder::TaskActions(
    const std::string& id, domain::TaskStatus status) {
    dto::InlineKeyboardMarkup kb;
    if (status == domain::TaskStatus::kNew ||
        status == domain::TaskStatus::kPaused) {
        kb.push_back({Btn{"▶️ В работу", "task:progress:" + id, ""},
                      Btn{"✅ Готово",   "task:done:" + id, ""}});
    } else if (status == domain::TaskStatus::kInProgress) {
        kb.push_back({Btn{"⏸ Пауза",   "task:pause:" + id, ""},
                      Btn{"✅ Готово",  "task:done:" + id, ""}});
    }
    kb.push_back({Btn{"✏️ Изменить", "task:edit:" + id, ""},
                  Btn{"🗑 Удалить",  "task:delete:" + id, ""}});
    kb.push_back({Btn{"🔙 Назад",   "menu:tasks", ""}});
    return kb;
}

dto::InlineKeyboardMarkup KeyboardBuilder::SessionControls(
    const std::string& id, domain::SessionStatus status) {
    dto::InlineKeyboardMarkup kb;
    if (status == domain::SessionStatus::kActive) {
        kb.push_back({Btn{"⏸ Пауза",   "focus:pause:" + id, ""},
                      Btn{"🛑 Стоп",    "focus:stop:" + id, ""}});
...
dto::InlineKeyboardMarkup KeyboardBuilder::ConfirmCancel(
    const std::string& confirm_cb, const std::string& cancel_cb) {
    return {{Btn{"✅ Да", confirm_cb, ""},
             Btn{"❌ Нет", cancel_cb, ""}}};
}

dto::InlineKeyboardMarkup KeyboardBuilder::WeeklyReviewActions() {
    return {
        {Btn{"📊 Подробнее", "review:details",  ""},
         Btn{"📤 Экспорт",   "review:export",   ""}},
