...
ParsedQuickInput CommandParser::ParseQuickInput(const std::string& text) {
    ...
    if (auto p = core::ExtractPriorityHint(text)) {
        switch (*p) {
            case 1: result.priority = domain::TaskPriority::kCritical; break;
            case 2: result.priority = domain::TaskPriority::kHigh;     break;
            case 3: result.priority = domain::TaskPriority::kMedium;   break;
            case 4: result.priority = domain::TaskPriority::kLow;      break;
        }
    }
    ...
    title = std::regex_replace(title, std::regex(R"(\b(tomorrow|today|\d{1,2}[./]\d{1,2}|\d{2}:\d{2})\b)", std::regex::icase), "");
...
std::optional<std::string> CommandParser::ExtractTaskId(const std::string& text) {
    ...
    std::smatch m;
    if (std::regex_search(text, m, id_re)) return m[1].str();
    return std::nullopt;
}
...
std::optional<int> CommandParser::ParseDuration(const std::string& text) {
    ...
    std::smatch m;
    if (!std::regex_search(text, m, dur_re)) return std::nullopt;
    int total = 0;
    try {
        if (m[1].matched && m[1].length() > 0) total += std::stoi(m[1].str()) * 60;
        if (m[2].matched && m[2].length() > 0) total += std::stoi(m[2].str());
    } catch (const std::exception&) {
        return std::nullopt;  // переполнение или некорректное число
    }
    // Ограничиваем разумным максимумом (8 часов)
    if (total <= 0 || total > 480) return std::nullopt;
    return total;
}
