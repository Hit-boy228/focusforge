...
ReminderScene::ReminderScene(const userver::components::ComponentConfig& cfg,
...
void ReminderScene::HandleText(const dto::TgMessage& msg, const std::string& state) {
    if (state == "REMINDER_TEXT") {
        ...
        try {
            ...
            notify_.SendMessage(msg.chat.id,
                                ReplyBuilder::OkReply(msg.chat.id, "Напоминание установлено на " +
                                                                       core::FormatHuman(*tp))
                                    .text);
