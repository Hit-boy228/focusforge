...
EditTaskScene::EditTaskScene(const userver::components::ComponentConfig& cfg,
...
void EditTaskScene::Start(const dto::TgMessage& msg, const std::string& task_id) {
    userver::formats::json::ValueBuilder data;
    data["task_id"] = task_id;
    conv_.SetState(msg.from.id, "EDIT_TASK_TITLE", data.ExtractValue());
    notify_.SendMessage(msg.chat.id, "✏️ Введи новое название задачи (или /skip чтобы не менять):");
}

void EditTaskScene::HandleText(const dto::TgMessage& msg, const std::string& state) {
    if (state != "EDIT_TASK_TITLE")
        return;
    ...
    if (task_id.empty() || msg.text == "/skip")
        return;
