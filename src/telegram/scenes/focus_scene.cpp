...
FocusScene::FocusScene(const userver::components::ComponentConfig& cfg,
...
void FocusScene::HandleText(const dto::TgMessage& msg, const std::string& state) {
    if (state == "FOCUS_CUSTOM_DURATION") {
        auto dur = CommandParser::ParseDuration(msg.text);
        if (!dur) {
            notify_.SendMessage(msg.chat.id, "❌ Не понял длительность. Пример: <code>45</code>");
            return;
        }
...
void FocusScene::HandleStop(const dto::TgMessage& msg) {
    ...
    auto reply =
        ReplyBuilder::ConfirmDialog(msg.chat.id, "Остановить активную сессию?",
                                    "focus:stop:" + active->id + ":confirm", "focus:cancel");
    notify_.SendMessage(msg.chat.id, reply.text);
}
...
void FocusScene::HandlePause(const dto::TgMessage& msg) {
    ...
    if (!active->IsActive()) {
        ...
        try {
            ...
            notify_.SendMessage(msg.chat.id, ReplyBuilder::ErrorReply(msg.chat.id, e.what()).text);
        }
        return;
    }
    // Ставим на паузу
...
void FocusScene::HandleModeCallback(const dto::TgCallbackQuery& cq, const std::string& mode) {
    ...
    userver::formats::json::ValueBuilder data;
    data["mode"] = mode;
    conv_.SetState(cq.from.id, "FOCUS_STARTING", std::make_optional(data.ExtractValue()));
...
void FocusScene::AskMode(int64_t chat_id, int64_t user_id) {
    ...
    auto req = ReplyBuilder::Prompt(chat_id, messages::kAskFocusMode,
...
void FocusScene::LaunchSession(int64_t chat_id, int64_t tg_user_id) {
    ...
    if (data) {
        ...
        try {
            req.mode = domain::SessionModeFromString(mode_str);
        } catch (...) {
        }
