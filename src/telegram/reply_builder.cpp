...
dto::SendMessageRequest ReplyBuilder::MainMenu(int64_t chat_id, const std::string& user_name) {
...
dto::SendMessageRequest ReplyBuilder::ErrorReply(int64_t chat_id, const std::string& message) {
    {
        dto::SendMessageRequest req;
        req.chat_id = chat_id;
        req.text = "❌ " + core::EscapeHtml(message);
        return req;
    }
}

dto::SendMessageRequest ReplyBuilder::OkReply(int64_t chat_id, const std::string& message) {
    {
        dto::SendMessageRequest req;
        req.chat_id = chat_id;
        req.text = "✅ " + core::EscapeHtml(message);
...
dto::SendMessageRequest ReplyBuilder::ConfirmDialog(int64_t chat_id, const std::string& text,
    ...
    return [&] {
        dto::SendMessageRequest req;
        req.chat_id = chat_id;
        req.text = "⚠️ " + core::EscapeHtml(text);
        req.parse_mode = "HTML";
        req.reply_markup = kb;
        return req;
    }();
...
dto::SendMessageRequest ReplyBuilder::TaskList(int64_t chat_id,
                                               const std::vector<domain::Task>& tasks, int total,
                                               const std::string& title) {
    std::string text = "<b>" + core::EscapeHtml(title.empty() ? "Мои задачи" : title) + "</b> (" +
                       std::to_string(total) + ")\n\n";
    for (int i = 0; i < static_cast<int>(tasks.size()); ++i)
        text += FormatTaskLine(tasks[i], i + 1) + "\n";
    if (tasks.empty())
        text += "📭 Задач нет. Создай первую!";
    return [&] {
        dto::SendMessageRequest req;
        req.chat_id = chat_id;
        req.text = text;
...
dto::SendMessageRequest ReplyBuilder::SessionCard(int64_t chat_id, const domain::FocusSession& s) {
...
dto::SendMessageRequest ReplyBuilder::DailyStats(int64_t chat_id, const domain::DailyStats& stats) {
...
dto::SendMessageRequest ReplyBuilder::Prompt(int64_t chat_id, const std::string& text,
...
std::string ReplyBuilder::FormatTaskLine(const domain::Task& t, int index) {
    std::string line = std::to_string(index) + ". " + domain::StatusEmoji(t.status) + " " +
                       domain::PriorityEmoji(t.priority) + " " + "<b>" +
                       core::EscapeHtml(core::Truncate(t.title, 50)) + "</b>";
    if (t.deadline) {
