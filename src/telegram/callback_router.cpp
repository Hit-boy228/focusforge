...
void CallbackRouter::Route(const dto::TgCallbackQuery& cq) {
    ...
    if (parts.empty()) { AnswerCallback(cq.id); return; }
    ...
    try {
        ...
        if      (parts[0] == "task")     HandleTaskCallback(cq, parts);
        else if (parts[0] == "focus")    HandleFocusCallback(cq, parts);
        else if (parts[0] == "reminder") HandleReminderCallback(cq, parts);
        else if (parts[0] == "menu")     HandleMenuCallback(cq, parts);
        AnswerCallback(cq.id);
...
void CallbackRouter::HandleReminderCallback(const dto::TgCallbackQuery& cq,
    ...
    if (action == "snooze") {
        int minutes = 60;
        if (parts.size() > 3) {
            try { minutes = std::stoi(parts[3]); } catch (...) {}
        }
        ...
        AnswerCallback(cq.id,
            "Отложено на " + std::to_string(minutes) + " мин ✅");
...
void CallbackRouter::HandleMenuCallback(const dto::TgCallbackQuery& cq,
    ...
    std::string hint;
    if (action == "tasks")    hint = "Используй /tasks";
    else if (action == "focus")    hint = "Используй /focus";
    else if (action == "stats")    hint = "Используй /stats";
    else if (action == "plan")     hint = "Используй /plan";
    else if (action == "settings") hint = "Используй /settings";
    if (!hint.empty())
        notification_service_.SendMessage(cq.message.chat.id, hint);
}

void CallbackRouter::AnswerCallback(const std::string& /*id*/,
