...
std::optional<core::ValidationError> TaskValidator::ValidateCreate(
    ...
    if (req.deadline_iso) {
        if (auto e = ValidateDeadlineIso(*req.deadline_iso))
            return e;
    }
    return std::nullopt;
}
...
std::optional<core::ValidationError> TaskValidator::ValidateTagName(const std::string& name) {
...
std::optional<core::ValidationError> TaskValidator::ValidateDeadlineIso(const std::string& iso) {
