...
std::optional<core::ValidationError> UserValidator::ValidateRegister(
    ...
    if (!req.timezone.empty()) {
        if (auto e = ValidateTimezone(req.timezone))
            return e;
    }
    return std::nullopt;
}
...
std::optional<core::ValidationError> UserValidator::ValidateUpdateSettings(
    ...
    auto check_minutes = [](std::optional<int> v, const char* field, int min,
                            int max) -> std::optional<core::ValidationError> {
        if (!v)
            return std::nullopt;
        if (*v < min || *v > max)
            return core::ValidationError(
                field, "must be " + std::to_string(min) + "–" + std::to_string(max));
        return std::nullopt;
    };
    if (auto e = check_minutes(req.pomodoro_work_minutes, "pomodoro_work_minutes", 5, 120))
        return e;
    if (auto e = check_minutes(req.pomodoro_break_minutes, "pomodoro_break_minutes", 1, 60))
        return e;
    if (auto e =
            check_minutes(req.pomodoro_long_break_minutes, "pomodoro_long_break_minutes", 5, 60))
        return e;
    if (auto e = check_minutes(req.deep_work_minutes, "deep_work_minutes", 15, 300))
        return e;
    if (req.timezone) {
        if (auto e = ValidateTimezone(*req.timezone))
            return e;
...
std::optional<core::ValidationError> UserValidator::ValidateTimezone(const std::string& tz) {
