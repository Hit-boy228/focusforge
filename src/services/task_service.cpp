...
TaskService::TaskService(const userver::components::ComponentConfig& cfg,
...
domain::Task TaskService::CreateTask(const dto::CreateTaskRequest& req) {
    ...
    auto trx = pg->Begin("create_task", userver::storages::postgres::ClusterHostType::kMaster, {});
    ...
    if (req.idempotency_key) {
        ...
        cache_.Set("idem:" + *req.idempotency_key,
                   userver::formats::json::ToString(b.ExtractValue()), std::chrono::seconds(86400));
    }
...
domain::Task TaskService::GetTask(const std::string& task_id, const std::string& user_id) {
    auto task = task_repo_.FindById(task_id, user_id);
    if (!task)
        throw core::NotFoundError("task", task_id);
...
domain::Task TaskService::UpdateTask(const dto::UpdateTaskRequest& req) {
    ...
    // Применяем изменения
    if (req.title)
        current->title = core::Trim(*req.title);
    if (req.description)
    ...
    if (req.estimated_minutes)
        current->estimated_minutes = *req.estimated_minutes;
    ...
    auto pg = task_repo_.GetCluster();
    auto trx = pg->Begin("update_task", userver::storages::postgres::ClusterHostType::kMaster, {});
    auto updated = task_repo_.UpdateWithVersion(trx, *current, req.expected_version);
    trx.Commit();
    lock_repo_.Release(lock_key);

    if (!updated)
        throw core::ConflictError("Version mismatch for task " + req.task_id + " (expected " +
                                  std::to_string(req.expected_version) + ")");
...
domain::Task TaskService::ChangeStatus(const std::string& task_id, const std::string& user_id,
    ...
    auto pg = task_repo_.GetCluster();
    auto trx =
        pg->Begin("change_status", userver::storages::postgres::ClusterHostType::kMaster, {});
    auto updated = task_repo_.UpdateWithVersion(trx, *current, current->version);
    trx.Commit();

    if (!updated)
        throw core::ConflictError("Version mismatch for task " + task_id);
...
void TaskService::DeleteTask(const dto::DeleteTaskRequest& req) {
    ...
    auto pg = task_repo_.GetCluster();
    auto trx = pg->Begin("delete_task", userver::storages::postgres::ClusterHostType::kMaster, {});
...
domain::Subtask TaskService::AddSubtask(const dto::CreateSubtaskRequest& req) {
    ...
    auto pg = task_repo_.GetCluster();
    auto trx = pg->Begin("add_subtask", userver::storages::postgres::ClusterHostType::kMaster, {});
...
void TaskService::ToggleSubtask(const dto::ToggleSubtaskRequest& req) {
    auto pg = task_repo_.GetCluster();
    auto trx =
        pg->Begin("toggle_subtask", userver::storages::postgres::ClusterHostType::kMaster, {});
...
domain::InboxItem TaskService::AddInboxItem(const dto::AddInboxItemRequest& req) {
    ...
    if (priority_hint) {
        switch (*priority_hint) {
            case 1:
                item.parsed_priority = domain::TaskPriority::kCritical;
                break;
            case 2:
                item.parsed_priority = domain::TaskPriority::kHigh;
                break;
            case 3:
                item.parsed_priority = domain::TaskPriority::kMedium;
                break;
            case 4:
                item.parsed_priority = domain::TaskPriority::kLow;
                break;
        }
    }
    ...
    LOG_INFO() << "Inbox item added: user=" << req.user_id << " text_len=" << req.raw_text.size();
    return item;
}
...
domain::Task TaskService::ProcessInboxItem(const dto::ProcessInboxItemRequest& req) {
    ...
    if (req.deadline_iso)
        create_req.deadline_iso = *req.deadline_iso;
...
void TaskService::WriteHistory(const std::string& task_id, const std::string& user_id,
                               const std::string& action, const std::optional<std::string>& field,
