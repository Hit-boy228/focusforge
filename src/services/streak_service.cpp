...
StreakService::StreakService(const userver::components::ComponentConfig& cfg,
...
domain::Streak StreakService::UpdateStreak(const std::string& user_id) {
    ...
    if (consecutive || streak.current_streak == 0) {
        ...
        bool frozen =
            streak.streak_frozen_until.has_value() && *streak.streak_frozen_until >= today;
    ...
    LOG_INFO() << "Streak updated: user=" << user_id << " current=" << streak.current_streak;
    return streak;
}

bool StreakService::UseGraceDay(const std::string& user_id) {
    auto streak = user_repo_.GetStreak(user_id);
    if (streak.grace_days_used >= streak.grace_days_total)
        return false;
    streak.grace_days_used++;
    user_repo_.UpdateStreak(streak);
    return true;
...
void StreakService::FreezeStreak(const std::string& user_id, const std::string& until_date) {
    auto streak = user_repo_.GetStreak(user_id);
