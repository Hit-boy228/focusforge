...
void ConversationService::SetState(
    int64_t user_id,
    ...
    if (data) {
        ...
        if (existing) cs.data = existing->data;
        else {
...
std::optional<userver::formats::json::Value> ConversationService::GetData(
    int64_t user_id) {
    auto s = conv_repo_.Get(user_id);
...
void ConversationService::UpdateData(
    int64_t user_id,
    ...
    userver::formats::json::ValueBuilder b(
        userver::formats::json::Type::kObject);
    if (existing) b = userver::formats::json::ValueBuilder{existing->data};
    b[key] = value;
