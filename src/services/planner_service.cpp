...
DayPlan PlannerService::BuildDayPlan(
    ...
    for (auto& task : tasks) {
        if (task.status == domain::TaskStatus::kDone ||
            task.status == domain::TaskStatus::kArchived) continue;

        const int need = task.estimated_minutes.value_or(30);
        if (remaining < 15) break;  // Меньше 15 мин — не начинаем
    ...
    if (plan.ordered_tasks.empty()) {
        ...
        int overdue = 0;
        for (const auto& t : plan.ordered_tasks) if (t.IsOverdue()) ++overdue;
        if (overdue > 0)
            plan.advice_text = "⚠️ " + std::to_string(overdue)
                + " просроченных задач. Начни с них!";
        else
            plan.advice_text = "✅ " + std::to_string(plan.ordered_tasks.size())
                + " задач запланировано. Удачи!";
    }
...
double PlannerService::ScoreTask(const domain::Task& task, int available_minutes) {
    ...
    // Просрочена → +100
    if (task.IsOverdue()) score += 100.0;

    // Дедлайн
    if (task.deadline) {
        const auto now  = domain::Now();
        const auto days = std::chrono::duration_cast<std::chrono::hours>(
            *task.deadline - now).count() / 24.0;
        if      (days < 1) score += 80.0;
        else if (days < 3) score += 40.0;
        else if (days < 7) score += 20.0;
    }
