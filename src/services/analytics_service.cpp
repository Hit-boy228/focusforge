...
domain::DailyStats AnalyticsService::GetDailyStats(
    ...
    if (stats.tasks_completed > 0)
        stats.completion_rate = std::min(1.0,
            static_cast<double>(stats.tasks_completed) / 5.0);  // 5 задач = 100%

    return stats;
}

domain::WeeklyReport AnalyticsService::GetWeeklyReport(
    ...
    const std::string week_end = core::FormatDate(
        *week_start_tp + std::chrono::hours(6 * 24));
...
domain::WeeklyReport AnalyticsService::BuildWeeklyReport(
    ...
    report.total_focus_minutes   = session_repo_.SumFocusMinutes(
        user_id, week_start, week_end);
    report.total_tasks_completed = task_repo_.CountCompletedInPeriod(
        user_id, week_start, week_end);
    ...
    if (report.total_tasks_completed > 0)
        report.task_completion_rate = std::min(1.0,
    ...
    if (week_start_tp) {
        for (int d = 0; d < 7; ++d) {
            const std::string day_str = core::FormatDate(
                *week_start_tp + std::chrono::hours(d * 24));
...
std::string AnalyticsService::ExportReport(const dto::ExportReportRequest& req) {
    ...
    const int focus = session_repo_.SumFocusMinutes(
        req.user_id, req.period_start, req.period_end);
    const int tasks = task_repo_.CountCompletedInPeriod(
        req.user_id, req.period_start, req.period_end);
