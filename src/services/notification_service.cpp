...
NotificationService::NotificationService(const userver::components::ComponentConfig& cfg,
...
void NotificationService::SendSessionStarted(int64_t chat_id, const domain::FocusSession& s) {
...
void NotificationService::SendSessionCompleted(int64_t chat_id, const domain::FocusSession& s) {
...
void NotificationService::SendReminder(int64_t chat_id, const domain::Reminder& r) {
    SendMessage(chat_id, "⏰ <b>Напоминание</b>\n\n" + core::EscapeHtml(r.message));
}

void NotificationService::SendWeeklyReport(int64_t chat_id, const domain::WeeklyReport& r) {
...
void NotificationService::PostToTelegramApi(const std::string& method,
    ...
    try {
        ...
        if (response->status_code() != 200) {
            LOG_WARNING() << "Telegram API error [" << method << "]: " << response->body_view();
        }
