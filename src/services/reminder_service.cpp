...
ReminderService::ReminderService(const userver::components::ComponentConfig& cfg,
                                 const userver::components::ComponentContext& ctx)
    : ComponentBase(cfg, ctx)
    , reminder_repo_(ctx.FindComponent<repositories::postgres::ReminderRepository>()) {}

domain::Reminder ReminderService::CreateReminder(const dto::CreateReminderRequest& req) {
...
void ReminderService::SnoozeReminder(const dto::SnoozeReminderRequest& req) {
    ...
    const auto snooze_until = core::NowUtc() + std::chrono::minutes(req.snooze_minutes);
    const auto until_iso = core::FormatIso8601(snooze_until);

    std::optional<std::string> reason_str;
    if (req.reason)
        reason_str = domain::ToString(*req.reason);
...
void ReminderService::ProcessDueReminders() {
    ...
    for (const auto& r : due) {
        try {
            ...
            LOG_WARNING() << "Failed to send reminder " << r.id << ": " << e.what();
        }
    }
}
