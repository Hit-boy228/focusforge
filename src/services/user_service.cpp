...
domain::User UserService::UpdateSettings(const dto::UpdateUserSettingsRequest& req) {
    ...
    if (!user) throw core::NotFoundError("user", req.user_id);

    auto& s = user->settings;
    if (req.daily_focus_goal_minutes)    s.daily_focus_goal_minutes    = *req.daily_focus_goal_minutes;
    if (req.weekly_focus_goal_minutes)   s.weekly_focus_goal_minutes   = *req.weekly_focus_goal_minutes;
    if (req.pomodoro_work_minutes)       s.pomodoro_work_minutes       = *req.pomodoro_work_minutes;
    if (req.pomodoro_break_minutes)      s.pomodoro_break_minutes      = *req.pomodoro_break_minutes;
    if (req.pomodoro_long_break_minutes) s.pomodoro_long_break_minutes = *req.pomodoro_long_break_minutes;
    if (req.deep_work_minutes)           s.deep_work_minutes           = *req.deep_work_minutes;
    if (req.timezone)                    s.timezone                    = *req.timezone;
...
void UserService::TouchUser(int64_t tg_id) {
    ...
    if (user) user_repo_.UpdateLastSeen(user->id);
}
...
domain::User UserService::GetCachedProfile(int64_t tg_id) {
    ...
    auto user = user_repo_.FindByTelegramId(tg_id);
    if (!user) throw core::NotFoundError("user", std::to_string(tg_id));
    ...
    cache_.Set(key, userver::formats::json::ToString(b.ExtractValue()),
               std::chrono::seconds(300));
    return *user;
}
